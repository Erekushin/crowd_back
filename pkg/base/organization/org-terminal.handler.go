...
type OrgTerminalHandler struct{}

func (*OrgTerminalHandler) List(c *fiber.Ctx) error {
	var (
		res interface{}
		err error
	)
	...
	if res, err = TerminalList(c, orgId); err != nil {
		return core.Resolve(500, c, core.Response(err.Error()))
	}
	return core.Resolve(200, c, core.Response("success", res))
}

func (*OrgTerminalHandler) Add(c *fiber.Ctx) error {
	data := new(OrgTerminal)
...
func (*OrgTerminalHandler) Remove(c *fiber.Ctx) error {
	data := new(OrgTerminal)
